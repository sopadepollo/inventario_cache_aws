...
const registrarMovimiento = async (req,res) => {
	...
	try{
		await enviarACola('MOVIMIENTO_INVENTARIO', payload);
		res.status(202).json({mensaje:'Inventario encolado'});
	}catch(error){
		res.status(500).json({error:'Error al encolar la actualizacion de inventario'});
	}
